...
cl_mem input,length,accumulator;
int result[3*34*34];
...
int initialize(){
    ...
    string binary_file = getBoardBinaryFile("kernel_tb1", device);
    printf("Using AOCX: %s\n", binary_file.c_str());
    ...
    //
    kernel[0] = clCreateKernel(program, "reduce_sum", &err);
    checkerror(err,"Error: Failed to create compute kernel[0]!");

    // Create the input and output arrays in device memory for our calculation
    //
    input = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(int) * 3 * 34 * 34, NULL, NULL);
    accumulator = clCreateBuffer(context, CL_MEM_WRITE_ONLY, sizeof(int) * 3 * 34 * 34, NULL, NULL);
...
void run(){
    ...
    //
    err = clEnqueueWriteBuffer(queue[0], input, CL_FALSE, 0, sizeof(int) * 3 * 34 * 34, data, 0, NULL, NULL);
     	checkerror(err,"Error: Failed to Write kernel arguments! - kernel[0] - data");

    // Set the arguments to our compute kernel
    //
    err = clSetKernelArg(kernel[0],0, sizeof(cl_mem), &input);
    checkerror(err,"Error: Failed to set kernel arguments! - kernel[0] - input");
    
    err = clSetKernelArg(kernel[0],1, sizeof(cl_mem), &accumulator);
	checkerror(err,"Error: Failed to set kernel arguments! - kernel[0] - accumulator");
    ...
    printf("Complete execute \n");
    clFinish(queue[0]);
    //clFinish(queue[1]);
    err = clEnqueueReadBuffer( queue[0] , accumulator, CL_TRUE, 0, sizeof(int) * 3 * 34 * 34, &result, 0, NULL, NULL );
    printf("Complete \n");
...
void cleanup(){
    ...
    clReleaseMemObject(length);
    clReleaseMemObject(accumulator);
