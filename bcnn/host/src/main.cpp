...
scoped_aligned_ptr<int> h_fmap2;

scoped_aligned_ptr<int> h_w3;
scoped_aligned_ptr<int> h_norm3;
scoped_aligned_ptr<int> h_act3;
scoped_aligned_ptr<int> h_fmap3;

scoped_aligned_ptr<int> h_w4;
scoped_aligned_ptr<int> h_norm4;
scoped_aligned_ptr<int> h_act4;
scoped_aligned_ptr<int> h_fmap4;

scoped_aligned_ptr<int> h_w5;
scoped_aligned_ptr<int> h_norm5;
scoped_aligned_ptr<int> h_act5;
scoped_aligned_ptr<int> h_fmap5;

scoped_aligned_ptr<int> h_w6;
scoped_aligned_ptr<int> h_norm6;
scoped_aligned_ptr<int> h_act6;
scoped_aligned_ptr<int> h_fmap6;

scoped_aligned_ptr<int> h_w7;
scoped_aligned_ptr<int> h_norm7;
scoped_aligned_ptr<int> h_act7;
scoped_aligned_ptr<int> h_fmap7;

scoped_aligned_ptr<int> h_w8;
scoped_aligned_ptr<int> h_norm8;
scoped_aligned_ptr<int> h_act8;
scoped_aligned_ptr<int> h_fmap8;

scoped_aligned_ptr<int> h_w9;
scoped_aligned_ptr<int> h_norm9;
scoped_aligned_ptr<int> h_act9;
scoped_aligned_ptr<int> h_fmap9;
...
//Layer 9
    ...
    string binary_file = getBoardBinaryFile("kernel_nomask_b2", device);
    printf("Using AOCX: %s\n", binary_file.c_str());
    ...
    d_fmap2 = clCreateBuffer(context, CL_MEM_READ_WRITE, sizeof(int) * 128 * 18 * 18, NULL, NULL);

    d_w3[0] = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(int) * 256 * 128 * 3 * 3, NULL, NULL);
    d_norm3[0] = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(int) * 256, NULL, NULL);
    d_dim3[0] = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(int)*3, NULL, NULL);
    d_act3 = clCreateBuffer(context, CL_MEM_WRITE_ONLY, sizeof(int) * 256 * 16 * 16, NULL, NULL);
    d_fmap3 = clCreateBuffer(context, CL_MEM_READ_WRITE, sizeof(int) * 256 * 18 * 18, NULL, NULL);
    
    h_dim3 = {256,17,17};

    d_w4[0] = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(int) * 256 * 256 * 3 * 3, NULL, NULL);
    d_norm4[0] = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(int) * 256, NULL, NULL);
    d_act4 = clCreateBuffer(context, CL_MEM_WRITE_ONLY, sizeof(int) * 256 * 16 * 16, NULL, NULL);
    d_fmap4 = clCreateBuffer(context, CL_MEM_READ_WRITE, sizeof(int) * 256 * 10 * 10, NULL, NULL);

    d_w5[0] = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(int) * 512 * 256 * 3 * 3, NULL, NULL);
    d_norm5[0] = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(int) * 512, NULL, NULL);
    d_dim5[0] = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(int) * 3, NULL, NULL);
    d_act5 = clCreateBuffer(context, CL_MEM_WRITE_ONLY, sizeof(int) * 512 * 8 * 8, NULL, NULL);
    d_fmap5 = clCreateBuffer(context, CL_MEM_READ_WRITE, sizeof(int) * 512 * 10 * 10, NULL, NULL);
    
    h_dim5 = {512,9,9}; 

    d_w6[0] = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(int) * 512 * 512 * 3 * 3, NULL, NULL);
    d_norm6[0] = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(int) * 512, NULL, NULL);
    d_act6 = clCreateBuffer(context, CL_MEM_WRITE_ONLY, sizeof(int) * 512 * 8 * 8, NULL, NULL);
    d_fmap6 = clCreateBuffer(context, CL_MEM_READ_WRITE, sizeof(int) * 8192, NULL, NULL);

    d_w7[0] = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(int) * 8192 * 1024, NULL, NULL);
    d_norm7[0] = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(int) * 1024, NULL, NULL);
    d_act7 = clCreateBuffer(context, CL_MEM_WRITE_ONLY, sizeof(int) * 1024, NULL, NULL);
    d_fmap7 = clCreateBuffer(context, CL_MEM_READ_WRITE, sizeof(int) * 1024, NULL, NULL);

    d_w8[0] = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(int) * 1024 * 1024, NULL, NULL);
    d_norm8[0] = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(int) * 1024, NULL, NULL);
    d_act8 = clCreateBuffer(context, CL_MEM_WRITE_ONLY, sizeof(int) * 1024, NULL, NULL);
    d_fmap8 = clCreateBuffer(context, CL_MEM_READ_WRITE, sizeof(int) * 1024, NULL, NULL); 

    d_w9[0] = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(int) * 10 * 1024, NULL, NULL);
    d_norm9[0] = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(int) * 10, NULL, NULL);
    d_act9 = clCreateBuffer(context, CL_MEM_WRITE_ONLY, sizeof(int) * 10, NULL, NULL);
    d_fmap9 = clCreateBuffer(context, CL_MEM_READ_WRITE, sizeof(int) * 10, NULL, NULL);         
    ...
    global = {34, 34, 128};
        ...
        kernel[2] = clCreateKernel(program, "layerthree", &err);
        ...
        kernel[3] = clCreateKernel(program, "layerfour", &err);
...
//Layer 3        
        err = clEnqueueWriteBuffer(queue[2], d_w3[0], CL_FALSE, 0, sizeof(int) * 256 * 128 * 3 * 3, h_w3, 0, NULL, NULL);
        checkError(err, "Error: Failed to copy kernel arguments! - kernel[%d] - h_w3",2);

        err = clEnqueueWriteBuffer(queue[2], d_norm3[0], CL_FALSE, 0, sizeof(int) * 256, h_norm3, 0, NULL, NULL);
        checkError(err, "Error: Failed to copy kernel arguments! - kernel[%d] - h_norm3",2);

        err = clEnqueueWriteBuffer(queue[2], d_dim3[0], CL_FALSE, 0, sizeof(int) * 3, h_dim3, 0, NULL, NULL);
        checkError(err, "Error: Failed to copy kernel arguments! - kernel[%d] - h_dim3",2);

//Layer 4
        err = clEnqueueWriteBuffer(queue[3], d_w4[0], CL_FALSE, 0, sizeof(int) * 256 * 256 * 3 * 3, h_w4, 0, NULL, NULL);
        checkError(err, "Error: Failed to copy kernel arguments! - kernel[%d] - h_w4",3);

        err = clEnqueueWriteBuffer(queue[3], d_norm4[0], CL_FALSE, 0, sizeof(int) * 256, h_norm4, 0, NULL, NULL);
        checkError(err, "Error: Failed to copy kernel arguments! - kernel[%d] - h_norm4",3);

//Layer 5        
        err = clEnqueueWriteBuffer(queue[4], d_w5[0], CL_FALSE, 0, sizeof(int) * 512 * 256 * 3 * 3, h_w5, 0, NULL, NULL);
        checkError(err, "Error: Failed to copy kernel arguments! - kernel[%d] - h_w5",4);

        err = clEnqueueWriteBuffer(queue[4], d_norm5[0], CL_FALSE, 0, sizeof(int) * 512, h_norm5, 0, NULL, NULL);
        checkError(err, "Error: Failed to copy kernel arguments! - kernel[%d] - h_norm5",4);

        err = clEnqueueWriteBuffer(queue[4], d_dim5[0], CL_FALSE, 0, sizeof(int) * 3, h_dim5, 0, NULL, NULL);
        checkError(err, "Error: Failed to copy kernel arguments! - kernel[%d] - h_dim5",4);

//Layer 6
        err = clEnqueueWriteBuffer(queue[5], d_w6[0], CL_FALSE, 0, sizeof(int) * 512 * 512 * 3 * 3, h_w6, 0, NULL, NULL);
        checkError(err, "Error: Failed to copy kernel arguments! - kernel[%d] - h_w6",5);

        err = clEnqueueWriteBuffer(queue[5], d_norm6[0], CL_FALSE, 0, sizeof(int) * 512, h_norm6, 0, NULL, NULL);
        checkError(err, "Error: Failed to copy kernel arguments! - kernel[%d] - h_norm6",5);

//Layer 7
        err = clEnqueueWriteBuffer(queue[6], d_w7[0], CL_FALSE, 0, sizeof(int) * 8192 *1024, h_w7, 0, NULL, NULL);
        checkError(err, "Error: Failed to copy kernel arguments! - kernel[%d] - h_w7",6);

        err = clEnqueueWriteBuffer(queue[6], d_norm7[0], CL_FALSE, 0, sizeof(int) * 1024, h_norm7, 0, NULL, NULL);
        checkError(err, "Error: Failed to copy kernel arguments! - kernel[%d] - h_norm7",6);

//Layer 8
        err = clEnqueueWriteBuffer(queue[7], d_w8[0], CL_FALSE, 0, sizeof(int) * 1024 *1024, h_w8, 0, NULL, NULL);
        checkError(err, "Error: Failed to copy kernel arguments! - kernel[%d] - h_w8",7);

        err = clEnqueueWriteBuffer(queue[7], d_norm8[0], CL_FALSE, 0, sizeof(int) * 1024, h_norm8, 0, NULL, NULL);
        checkError(err, "Error: Failed to copy kernel arguments! - kernel[%d] - h_norm8",7); 

//Layer 9
        err = clEnqueueWriteBuffer(queue[8], d_w9[0], CL_FALSE, 0, sizeof(int) * 10 *1024, h_w9, 0, NULL, NULL);
        checkError(err, "Error: Failed to copy kernel arguments! - kernel[%d] - h_w9",8);

        err = clEnqueueWriteBuffer(queue[8], d_norm9[0], CL_FALSE, 0, sizeof(int) * 10, h_norm9, 0, NULL, NULL);
...
//Layer 1
        ...
        checkError(err, "Error: Failed to set kernel arguments! - kernel[%d] - d_fmap1",0);
//Layer 2
        argi = 0;
...
//Layer 9
    ...
    err = clEnqueueReadBuffer(queue[8], d_fmap9, CL_FALSE, 0, sizeof(int) * 10, h_fmap9, 0, NULL, NULL);
