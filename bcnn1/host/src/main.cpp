...
//Layer 9
    ...
    cl_event event_kernel[N];
        ...
        //
        kernel[0] = clCreateKernel(program, "layerone", &err);
...
//Layer 1
        ...
        err = clEnqueueWriteBuffer(queue[0], d_w1[0], CL_FALSE, 0, sizeof(char) * 128 * 3 * 3 * 3, h_w1, 0, NULL, NULL);
        checkError(err, "Error: Failed to copy kernel arguments! - kernel[%d] - h_w1",0);

        err = clEnqueueWriteBuffer(queue[0], d_norm1[0], CL_FALSE, 0, sizeof(short) * 128, h_norm1, 0, NULL, NULL);
